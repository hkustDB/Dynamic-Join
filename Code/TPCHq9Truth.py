...
def loaddata(argv):
    ...
    try:
        ...
        print("TPCHq9Truth.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -d <theta(default 1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")> -o <psql_port(default 5432)>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-help':
            print("TPCHq9Truth.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -d <theta(default 1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")> -o <psql_port(default 5432)>")
...
def main(argv):
    ...
    for relation in relation_list:
        intermediate_file_path = '../Temp/TPCH/intermediate/q9_' + relation + '_truth_' + str(rounds) + '.txt'
    ...
    TPCH = TPCHq9Truth(relation_list, T, query_name, query)
    writeline = "dataset=TPCH, query_name=q9, (Intervals)" + '\n'
