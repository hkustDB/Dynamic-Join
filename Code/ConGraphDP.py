...
class ConGraphDP:
    ...
    def Mf_List(self, query_name):
        if query_name in ("two_path", "two_star"):
            return [1]*relation_nums
        elif query_name in ("three_path", "triangle", "rectangle"):
            return [2]*relation_nums
...
def loaddata(argv):
    ...
    try:
        ...
        print("ConGraphDP.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -d <theta(default 1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-help':
            print("ConGraphDP.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -d <theta(default 1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
            ...
            if query_name in ("two_star", "two_path"):
                ...
                print("query_name is in wrong domain, please make sure -q being correct")
                sys.exit()
