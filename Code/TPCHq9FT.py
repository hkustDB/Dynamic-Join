...
def loaddata(argv):
    ...
    try:
        ...
        print("TPCHq9FT.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-help':
            print("TPCHq9FT.py -d <dataset> -q <query name> -e <epsilon(default 1)> -b <beta(default 0.1)> -T <tau(default 16)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
    ...
    answer_file_path = "../Temp/" + dataset + '/answer/' + 'q9_FixTau_' + str(tau) + '_' + str(rounds) + '_' + currentTime + '.txt'
...
def main(argv):
    ...
    for relation in relation_list:
        intermediate_file_path = '../Temp/TPCH/intermediate/q9_' + relation + '_FixTau_' + str(tau) + '_' + str(rounds) + '.txt'
    ...
    TPCH = TPCHq9FT(relation_list, epsilon, beta, T, query_name, query)
    writeline = "dataset=TPCH, query_name=q9, epsilon=%s, beta=%s, tau=%s, rounds=%s, (Intervals)"%(epsilon, beta, tau, rounds) + '\n'
