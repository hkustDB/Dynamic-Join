...
class TPCHq7RS:
    ...
    def Query_Instance(self, query):
        ...
        for relation in self.relation_list:
            query_instance.replace(relation, relation+'_q7')
    ...
    def CreateTables(self):
        create_temp_table = "create table customer_q7 (ck integer not null)"
        self.cur.execute(create_temp_table)
        create_temp_table = "create table orders_q7 (ck integer not null, ok integer not null)"
        self.cur.execute(create_temp_table)
        create_temp_table = "create table lineitem_q7 (ok integer not null, sk integer not null)"
        self.cur.execute(create_temp_table)
        create_temp_table = "create table supplier_q7 (sk integer not null)"
        self.cur.execute(create_temp_table)
        self.con.commit()

    def DropTables(self):
        drop_table = "drop table customer_q7"
        self.cur.execute(drop_table)
        drop_table = "drop table orders_q7"
        self.cur.execute(drop_table)
        drop_table = "drop table lineitem_q7"
        self.cur.execute(drop_table)
        drop_table = "drop table supplier_q7"
        self.cur.execute(drop_table)
        self.con.commit()
    ...
    def GetResidualSensitivity(self, footstep, random_list):
        ...
        for t in tqdm(range(1, self.T+1)):
            if random_list[t-1] == 'C': C_count += 1
            elif random_list[t-1] == 'O': O_count += 1
            elif random_list[t-1] == 'L': L_count += 1
            elif random_list[t-1] == 'S': S_count += 1
            if t % footstep == 0:
                with open('../Temp/TPCH/customer.tbl', 'r') as f:
                    reader = csv.reader(f, delimiter='|')
                    data = [(row[0],) for i, row in enumerate(reader) if i < C_count]
                self.cur.executemany("insert into customer_q7 values (%s)", data)
                self.con.commit()
                with open('../Temp/TPCH/orders.tbl', 'r') as f:
                    reader = csv.reader(f, delimiter='|')
                    data = [(row[1], row[0]) for i, row in enumerate(reader) if i < O_count]
                self.cur.executemany("insert into orders_q7 values (%s, %s)", data)
                self.con.commit()
                with open('../Temp/TPCH/lineitem.tbl', 'r') as f:
                    reader = csv.reader(f, delimiter='|')
                    data = [(row[0], row[2]) for i, row in enumerate(reader) if i < L_count]
                self.cur.executemany("insert into lineitem_q7 values (%s, %s)", data)
                self.con.commit()
                with open('../Temp/TPCH/supplier.tbl', 'r') as f:
                    reader = csv.reader(f, delimiter='|')
                    data = [(row[0],) for i, row in enumerate(reader) if i < S_count]
                self.cur.executemany("insert into supplier_q7 values (%s)", data)
                self.con.commit()
                start = time.time()
                ...
                truncate_table = "truncate table customer_q7"
                self.cur.execute(truncate_table)
                truncate_table = "truncate table orders_q7"
                self.cur.execute(truncate_table)
                truncate_table = "truncate table lineitem_q7"
                self.cur.execute(truncate_table)
                truncate_table = "truncate table supplier_q7"
                self.cur.execute(truncate_table)
                self.con.commit()
    ...
    def WriteResult(self, answer_file_path, rounds, time_interval):
        ...
        writeline = "dataset=%s, query_name=%s, epsilon=%s, beta=%s, round=%s (RS)"%(dataset, query_name, epsilon, self.beta, rounds) + '\n'
...
def loaddata(argv):
    ...
    try:
        ...
        print("TPCHq7RS.py -d <dataset> -q <query name> -e <epsilon(default 1)> -D <Delta (default 10^-10)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-help':
            print("TPCHq7RS.py -d <dataset> -q <query name> -e <epsilon(default 1)> -D <Delta (default 10^-10)> -r <rounds(default 1)> -u <psql_user(default postgres)> -p <psql_password(default postgres)> -h <psql_host(defalut "")>")
...
def main(argv):
    ...
    beta = epsilon / 10
    CGRS = TPCHq7RS(relation_list, epsilon, beta, T, query_name)
