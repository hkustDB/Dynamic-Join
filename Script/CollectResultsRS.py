...
def main(argv):
    ...
    try:
        ...
        print("CollectResultsRS.py -d <dataset> -q <query_name>")
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-d", "--dataset"):
            dataset = str(arg)
    ...
    cmd_pool = Pool(processes=1)
    for rounds in range(num_rounds):
        if dataset in ["roadNet-CA", "dimacs9-USA", "wikipedia_link_nl", "wikipedia_link_ca", "dblp_coauthor", "flickr", "sx-stackoverflow"]:
            cmd = "python ../Code/ConGraphRS.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
        elif dataset in ["TPCH"]:
            if query_name in ["q7"]:
                cmd = "python ../Code/TPCHq7RS.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
            elif query_name in ["q9"]:
                cmd = "python ../Code/TPCHq9RS.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
