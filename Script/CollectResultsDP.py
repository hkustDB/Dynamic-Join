...
def main(argv):
    ...
    try:
        ...
        print("CollectResultsDP.py -d <dataset> -q <query_name>")
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-d", "--dataset"):
            dataset = str(arg)
    ...
    for rounds in range(num_rounds):
        if dataset in ["roadNet-CA", "dimacs9-USA", "wikipedia_link_nl", "wikipedia_link_ca", "dblp_coauthor", "flickr", "sx-stackoverflow"]:
            if query_name in ["two_path", "three_path", "triangle"]:
                cmd = "python ../Code/ConGraphDP.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
            elif query_name in ["three_star", "four_star"]:
                    cmd = "python ../Code/ConGraphDP\(Star\).py -d %s -q %s -r %s"%(dataset, query_name, rounds)
        elif dataset in ["TPCH"]:
            if query_name in ["q7"]:
                cmd = "python ../Code/TPCHq7DP.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
            elif query_name in ["q9"]:
                cmd = "python ../Code/TPCHq9DP.py -d %s -q %s -r %s"%(dataset, query_name, rounds)
