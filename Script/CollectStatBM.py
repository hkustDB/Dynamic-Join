...
def CollectBMError(answer_file_prefix):
    res_path = answer_file_prefix + 'BM/' + query_name + '/'
    ...
    stat_file = open('../Stat/' + dataset + '_' + query_name + '_BM_stat.txt', 'w')
    writeline = 'dataset=%s, query_name=%s (BM)'%(dataset, query_name)
    ave_error_list = []
    ...
    stat_file.write("median Relative Error:%e"%sorted(clipped_ave_relative_error_list)[int(0.5*len(clipped_ave_relative_error_list))] + '\n')
...
def main(argv):
    ...
    CollectBMError(answer_file_prefix)
