...
def main(argv):
    ...
    try:
        ...
        print("CollectResultsCM.py -d <dataset> -q <query_name>")
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-d", "--dataset"):
            dataset = str(arg)
    ...
    tau_list = random.choices(tau_list, k=100)
    ...
    for rounds in range(num_rounds):
        if dataset in ["roadNet-CA", "dimacs9-USA", "wikipedia_link_nl", "wikipedia_link_ca", "dblp_coauthor", "flickr", "sx-stackoverflow"]:
            if query_name in ["two_path", "three_path", "triangle"]:
                cmd = "python ../Code/ConGraphFT.py -d %s -q %s -r %s -T %s"%(dataset, query_name, rounds, tau_list[rounds])
            elif query_name in ["three_star", "four_star"]:
                    cmd = "python ../Code/ConGraphFT\(Star\).py -d %s -q %s -r %s -T %s"%(dataset, query_name, rounds, tau_list[rounds])
        elif dataset in ["TPCH"]:
            if query_name in ["q7"]:
                cmd = "python ../Code/TPCHq7FT.py -d %s -q %s -r %s -T %s"%(dataset, query_name, rounds, tau_list[rounds])
            elif query_name in ["q9"]:
                cmd = "python ../Code/TPCHq9FT.py -d %s -q %s -r %s -T %s"%(dataset, query_name, rounds, tau_list[rounds])
